...
export 'package:flutter/foundation.dart' show factory, immutable, mustCallSuper, optionalTypeArgs, protected, required, visibleForTesting;
...
class BuildOwner {
  ...
  BuildOwner({this.onBuildScheduled, FocusManager? focusManager}) : focusManager = focusManager ?? (FocusManager()..registerGlobalHandlers());
  ...
  void _debugTrackElementThatWillNeedToBeRebuiltDueToGlobalKeyShenanigans(Element node, GlobalKey key) {
    ...
    final Set<GlobalKey> keys = _debugElementsThatWillNeedToBeRebuiltDueToGlobalKeyShenanigans!.putIfAbsent(node, () => HashSet<GlobalKey>());
    keys.add(key);
  }
  ...
  void debugFillProperties(DiagnosticPropertiesBuilder properties) {
    ...
    if (_dependencies != null && _dependencies!.isNotEmpty) {
      final List<DiagnosticsNode> diagnosticsDependencies =
          _dependencies!.map((InheritedElement element) => element.widget.toDiagnosticsNode(style: DiagnosticsTreeStyle.sparse)).toList();
    ...
    assert(() {
      if (owner!._debugBuilding) {
        ...
        if (!_debugAllowIgnoredCallsToMarkNeedsBuild) {
          ...
          if (owner!._debugCurrentBuildTarget != null)
            information.add(
                owner!._debugCurrentBuildTarget!.describeWidget('The widget which was currently being built when the offending call was made was'));
          throw FlutterError.fromParts(information);
        }
        assert(
...
abstract class RenderObjectElement extends Element {
  ...
  List<Element> updateChildren(List<Element> oldChildren, List<Widget> newWidgets, {Set<Element>? forgottenChildren, List<Object?>? slots}) {
    ...
    final List<Element> newChildren =
        oldChildren.length == newWidgets.length ? oldChildren : List<Element>.filled(newWidgets.length, _NullElement.instance);
